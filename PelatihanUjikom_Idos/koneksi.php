...
class koneksi
{
	...
	{
		$this->koneksi = mysqli_connect("localhost", "root", "", "pelatihan_ujikom_idos");
	...
	function get_matkul()
	{
		...
		$data = $this->koneksi->query("select * from tbl_matkul2");
		return $data;
	}

	function add_matkul($kd_matkul, $nama_matkul, $sks)
	{
		$this->koneksi->query("insert into tbl_matkul2 values('$kd_matkul','$nama_matkul','$sks')");
		return true;
	}

	function get_matkulById($kd_matkul)
	{
		$data = $this->koneksi->query("select * from tbl_matkul2 where kd_matkul='$kd_matkul'");
		return $data;
	}
	...
	{
		$this->koneksi->query("delete from tbl_matkul2 where kd_matkul='$kd_matkul'");
		return true;
	}
	...
	{
		$this->koneksi->query("UPDATE tbl_dosen set nama_dosen='$nama_dosen', alamat='$alamat' where kd_dosen='$kd_dosen'");
		return true;
	}
